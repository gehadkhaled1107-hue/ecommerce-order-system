...
from products import InsufficientStockError, InvalidQuantityError
...
class OrderProcessingManager():
    ...
    (Renamed from OrderManager to avoid clashing with products.py's
    ...
    # (products.py originally exposed add_order on its own OrderManager)
    ...
    def update_order_status(self, order_id, new_status):
        if order_id in self.orders:
            ...
            log_action("STATUS_UPDATE", f"Order {order_id} updated to {new_status}")
            return order
...
def calculate_cart_and_shipping_total(cart_items):
    ...
    shipping = reduce(lambda accumulator, item: accumulator + (item.product.calculate_shipping() * item.quantity),
                      cart_items, 0)
...
def checkout(customer, cart_items, order_manager):
    ...
    log_action("CHECKOUT", f"Order {order.order_id} created for {customer.name}, total ${grand_total:.2f}")
...
def cancel_order(order_manager, order_id):
    ...
    log_action("CANCEL", f"Order {order_id} cancelled")
